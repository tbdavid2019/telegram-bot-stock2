...
- **深度整合 Polymarket 前瞻機率**：自動引入鏈上真實押注機率對傳導情境進行加權，提升推論精準度。

### 7. 🔮 Polymarket 預測市場前瞻情報與總經定價 (`/pm [關鍵字]`)
- **全球最大預測市場真金白銀定價**：以鏈上資本真實押注機率，克服傳統民調或社交情緒的口水雜訊。
- **總經黑天鵝、利率政策與科技催化劑**：即時掌握聯準會降息幅度分布、美國經濟衰退機率、關稅政策、大選預期與重大商業科技進展（如 OpenAI 產品發表、IPO 等）。
- **2MD 專屬代理集群穿透**：全面經由 `2md.aiurl.tw` / `create360.ai` 轉發，100% 避開台灣 ISP DNS 劫持（Sinkhole 182.173.0.181），毫秒級極速回應。
- **視覺化進度條與成交量**：輸出百分比進度條（如 `[████░░░░] 45.5%`）、24 小時成交額、流動性深度與原始合約連結。
- **動態延伸按鈕**：查詢後附帶快捷推薦按鈕（聯準會利率路徑、2026 經濟衰退、AI 科技催化劑、利率傳導鏈）。

### 8. 📐 量化研究與 Fama-French 多因子模型 (`/ff` / `/sepa` / `/val` / `/earn` / `/corr`)
...
- **多格式智慧解析與數據提取**：
  ...
  - **Excel 財務試算表 (`openpyxl` / `xlrd>=2.0.2,<3.0.0`)**：支援 `.xlsx` 與舊式 `.xls`，自動遍歷活頁簿工作表並擷取預覽表格；損毀或不支援格式會安全回傳解析錯誤。
...
| 指令 | 說明 | 範例 |
| :--- | :--- | :--- |
| **直接傳送文字** | 💬 智能金融助理自由問答 (整合量化模型、2MD 全網情報、傳導鏈、Polymarket 預測、快訊、三大法人籌碼、Fama-French 與 Wiki 發布) | `分析 2330.TW 基本面與技術指標` 或 `Polymarket 現在降息機率多少？` |
...
## 🧪 測試指南與 Telegram 提問範例 (Testing Guide & Query Examples)

您可以直接在 Telegram 與機器人的對話視窗中，使用以下兩種方式進行測試與驗證：
...
/start
```
> **說明**：重置上下文對話記憶，並向 Telegram 註冊最新的底部快捷選單與 BotFather 指令清單。

---

### 方式 A：使用「斜線指令」直接測試（精準、快速）

直接複製以下指令傳送給機器人，逐一驗證各項量化模型與功能：

| 測試功能 | 直接在 Telegram 輸入 | 預期返回內容 |
| :--- | :--- | :--- |
...
| 🧠 **Google TimesFM 500M 預測** | `/tfm 2330.TW` 或 `/tfm NVDA` | 該標的最新 TimesFM 預測目標價、預期漲幅與風報比；歷史時間支援 ISO／Unix 秒／Unix 毫秒／`YYYY/MM/DD` 與缺值安全排序；`/tfm top` 顯示排行榜。 |
...
| 📅 **美股重要財報日曆** | `/cal` | 近期美股重量級企業財報日程、EPS 預估與盤前/盤後標記。 |
| 📅 **全球宏觀經濟數據** | `/cal macro` | 全球央行決議、美國 CPI 通膨數據、非農就業 (NFP) 日程與前值預期。 |
| 📅 **聯準會點陣圖與利率** | `/cal fed` | CME FOMC 利率決議日程與降息預期。 |
| 📅 **大宗商品期貨結算** | `/cal commodities` | 原油、黃金等重大期貨結算與交割日曆。 |
| 🔮 **Polymarket 預測市場** | `/pm fed` 或 `/pm recession` | 鏈上真實資本押注的降息機率分布或美國經濟衰退定價進度條。 |

---

### 方式 B：用「自然語言」直接提問（測試 AI 助理動態工具調用）

無需輸入斜線指令，像與專業量化分析師對話一樣，直接傳送繁體中文日常句子：

- **詢問選股與進場時機**：
  > 🗣️ *「今天系統有沒有推薦哪些共振飆股可以留意？」*  
  > 🗣️ *「幫我看看最近有哪些股票符合季線或半年線拉回支撐的買點？」*
- **詢問大盤風險與部位配置**：
  > 🗣️ *「現在整體大盤宏觀體制安全嗎？建議我放幾成資金在股市？」*  
  > 🗣️ *「現在市場波動度高嗎？應該保守防守還是積極進攻？」*
- **詢問個股深度預測與風報比**：
  > 🗣️ *「用 TimesFM 幫我評估台積電 (2330.TW)，目標價跟停損價要設哪裡？」*  
  > 🗣️ *「輝達 NVDA 的風報比怎麼樣？預期漲幅大概多少？」*
- **詢問主力籌碼與神秘分點**：
  > 🗣️ *「台積電今天除了外資投信，有哪些特定的券商分點在重押買進？」*  
  > 🗣️ *「幫我查 3293 鈊象的三大法人跟主力券商分點進出狀況。」*
- **詢問重大財經事件與預測市場**：
  > 🗣️ *「這幾天美股有哪些重要公司要公布財報？」*  
  > 🗣️ *「幫我查 Polymarket 上現在預測聯準會降息的機率是多少？」*
