...
- **日股 (JPX) 與陸股 A 股 (SSE/SZSE) 官方清冊正式導入**：
  - **日本東京證券交易所 (`data/jpx_stock_registry.json`)**：收錄 **4,003 檔** Prime、Standard、Growth 與 ETF 標的，映射 `.T` 後綴，支援日文名稱與中文別名（如 `豐田` ➔ `7203.T`、`索尼` ➔ `6758.T`、`任天堂` ➔ `7974.T`、`軟銀` ➔ `9984.T`）。
  - **中國 A 股上交所與深交所 (`data/cn_stock_registry.json`)**：收錄 **2,009 檔** A 股與科創板標的，自動映射 `.SS` 與 `.SZ`，支援中文公司名稱與簡稱（如 `貴州茅台` ➔ `600519.SS`、`寧德時代` ➔ `300750.SZ`、`比亞迪A股` ➔ `002594.SZ`）。
  - **全球官方清冊總量突破 21,890 檔**：涵蓋 TWSE/TPEx (2,234)、全美 6 大交易所 (10,407)、HKEX (3,237)、JPX (4,003)、SSE/SZSE (2,009)。
- **落實「台北跨日首查觸發 + 併發收斂 + 災備降級」五星級每日更新架構 (`tools/stock.py`)**：
  - **日界線精準錨定**：以台北標準時區 `00:00 (Asia/Taipei)` 為每日更新分界，杜絕跨時區錯位。
  - **延遲求值 (Lazy On-Demand)**：不設半夜空轉定時任務，台北時間跨日後「第一次有人查詢時」（如 09:15）才觸發全球清冊最新檢索，無縫避開交易所半夜維護窗。
  - **全日共用 (24h Shared Cache)**：當天首次更新後，全日所有用戶與所有指令共用同一份本機快取，查詢耗時 0.0001 秒。
  - **併發收斂 (SingleFlight)**：跨日首發多個並發請求自動合併為單一任務，杜絕驚群效應。
  - **平滑降級 (Stale-While-Revalidate)**：若某交易所端點超時或連線重置，自動返回本機上一份資料並標記 `stale: True`，對話與查詢 0 報錯、0 中斷。
...
- **台股上市/上櫃全市場自動映射與內建離線備援 (`tools/stock.py`)**：
  - 徹底解決純數字股票代碼（如 `1476`, `2330`, `3293`, `0050`）在 Yahoo Finance 查無數據的嚴重問題。
  - 內建封裝包含 TWSE (上市 1,331 檔) 與 TPEx (上櫃 903 檔) 共 2,234 檔股票與中文全名稱對照表（`data/tw_stock_registry.json`），打包進 Docker 映像檔，具備 100% 零網路依賴（Offline-Ready）離線啟動能力。
  - 運行時採 Tier-1（內建清冊）-> Tier-2（法人機構日報動態快取更新）雙層架構，自動將代號精準映射為 `.TW` 或 `.TWO`，並支援中文公司名稱直接識別（如 `儒鴻` -> `1476.TW`、`鈊象` -> `3293.TWO`）。
...
- 修正 `/tfm <ticker>`：改讀取 888 Stock Quant 的單一標的預測歷史，只保留依標準化時間（ISO 8601／Unix 秒／Unix 毫秒／`YYYY/MM/DD` 斜線日期）倒序排列的 TimesFM 項目，缺少時間欄位時安全排到末端；`/tfm top` 維持排行榜模式。
...
- **新聞指令智慧整合 (`/n`)**：整併原先分散的 `/n` (美股新聞) 與 `/ny` (台股新聞)，支援自動識別台股（如 `2330`、`台積電`、`2330.TW`）與美股/全球市場（如 `TSLA`、`特斯拉`、`IBM`、`NVDA`、`SpaceX`）。
- **多來源智慧分流**：
  - 台股自動調用 2MD 繁體中文即時搜尋與 Yahoo 股市新聞備援。
  - 美股自動調用 2MD 財經快讀與 yfinance 即時新聞 API。
- **保留 `/ny` 相容別名**：`/ny` 自動轉發至 `/n` 智慧路由處理，確保舊使用者使用習慣不中斷。
- **自然語言對話無縫銜接**：主對話 Agent 深度整合 `get_financial_news` 與 `search_financial_web`，使用者無需記誦指令，直接在聊天中詢問「*台積電最近有什麼利多消息*」或「*特斯拉最新財經新聞*」，即可取得即時摘要與來源連結。
